...
Function Invoke-ISO8601Cleanup {
  ...
  # Only keep the number of backups specified by $KeepNum.  
  if ( $(Get-ChildItem -Path $BaseDir).Count -gt $KeepNum ) {
  Write-Host Performing log directory cleanup.
      $DeleteFolders = Get-ChildItem -Path $BaseDir -Filter $Filter | Sort-Object -Descending | Select-Object -Skip $KeepNum
      ...
      Write-Host "$KeepNum or fewer backups stored on disk. Nothing to delete."
  }
}
...
Write-Host "Cleaning up old backup directories." -ForegroundColor Cyan
Invoke-ISO8601Cleanup -KeepNum 3 -BaseDir $PRTGCertDir
